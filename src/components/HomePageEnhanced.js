...
const HomePageEnhanced = ({ 
  ...
  // Calculate statistics based on phase
  const retentionStats = calculateRetentionStats(auctionState.teams);
  const tradingStats = calculateTradingStats(auctionState.teams);
  
  const auctionStats = {
    total: auctionState.players.length,
    processed: auctionState.auctionHistory.length + auctionState.unsoldPlayers.length,
    sold: auctionState.auctionHistory.length,
    unsold: auctionState.unsoldPlayers.length
  };
  ...
  // Pre-auction (not started) view with phase introduction
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-600 via-emerald-600 to-green-800">
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          {/* Phase Stepper - HIGHLIGHT */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.2 }}
            className="bg-white/95 backdrop-blur-sm rounded-xl p-6 mb-8 shadow-xl"
          >
            <PhaseStepper 
              currentPhase={leaguePhase} 
              completedPhases={leaguePhase === 'trading' ? ['retention'] : leaguePhase === 'auction' ? ['retention', 'trading'] : []}
            />
          </motion.div>
          ...
          >
            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-lg border-l-4 border-teal-600">
              <div className="text-sm text-gray-600">Teams Ready</div>
              <div className="text-3xl font-bold text-teal-600">{Object.keys(auctionState.teams).length}</div>
              <p className="text-xs text-gray-500 mt-1">🏏 Cricket Teams</p>
            </div>

            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-lg border-l-4 border-blue-600">
              <div className="text-sm text-gray-600">Players Available</div>
              <div className="text-3xl font-bold text-blue-600">{auctionState.players.length}</div>
              <p className="text-xs text-gray-500 mt-1">📊 In Auction Pool</p>
            </div>

            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-lg border-l-4 border-purple-600">
              <div className="text-sm text-gray-600">Total Budget</div>
              <div className="text-3xl font-bold text-purple-600">🪙 {(auctionState.maxTokens * Object.keys(auctionState.teams).length).toLocaleString()}</div>
              <p className="text-xs text-gray-500 mt-1">💰 League Budget</p>
            </div>

            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-lg border-l-4 border-orange-600">
              ...
              <p className="text-xs text-gray-500 mt-1 capitalize">{leaguePhase}</p>
            </div>
          </motion.div>

          {/* Team Snapshots */}
          <TeamSnapshot teams={auctionState.teams} currentPhase={leaguePhase} />
          ...
          <motion.div 
            ...
            className="text-center mt-12 text-emerald-100 text-sm"
          >
            <p>🔄 Live updates every few seconds | 👥 Public viewing page</p>
            <p className="mt-2 text-xs">
              {leaguePhase === 'retention' && '📝 Submit retentions to lock in your core players'}
              {leaguePhase === 'trading' && '🤝 Propose and approve trades with other teams'}
              {leaguePhase === 'auction' && '⏳ Auction will start soon'}
            </p>
          </motion.div>
        </div>
      </div>
  ...
  // AUCTION PHASE - Original live auction view
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-600 via-emerald-600 to-green-800">
