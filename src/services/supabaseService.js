...
class SupabaseAuctionService {
  // Simple: Upload Excel data directly to Supabase
  async uploadExcelData(players, teams) {
    try {
      console.log('Uploading to Supabase:', { playersCount: players.length, teamsCount: teams.length });

      // 1. Clear existing data (optional - remove if you want to keep existing data)
      await supabase.from('players').delete().neq('id', 0);
      await supabase.from('teams').delete().neq('id', 0);

      // 2. Insert teams first
